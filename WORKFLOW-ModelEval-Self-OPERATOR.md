...
1. 定义本次评测目标（模型、轮次、是否重跑、超时）。
2. 决定评分口径（严格门控 / 执行优先 / 双轨并行）。
...
## 3) 轮次与样本结构（Operator 口径）

> 该结构由 Operator 定义，REVIEW 必须执行，不得降级。

- 每个样本必须执行：`round1 -> round2`
- 仅当 `HARD_ABORT` 才允许不进入 round2
- 推荐样本：A1（首次）+ A2（重派/复验）
...
- 未按结构执行（例如应有 B 却未 spawn）
- 仅口头进入下一轮，但无工具事件证据
- 把双轮降级成单轮且未经 Operator 许可
- 模型不一致且无重派/无解释
